...
void apply_to_pixel(int x, int y, IMAGE *original, IMAGE *result, FILTER *filter) {
	//TODO: Use Halos and don't ignore edges
	...
	#pragma acc parallel //present_or_copyin(filter[:1], original[:1])
	#pragma acc loop gang vector collapse(2) reduction(+:res_R, res_G, res_B)
