...
// Used strictly for sanity checking
...
/*	printf("\n rank= %d \n pabove = %d \n pbelow = %d \n",rank, pabove, pbelow);
	...
	printf("\n local height = %d \n local width = %d \n", local->height, local->width);
*/	
	MPI_Request req[nprocs*2]; //1 and 0 for pair of procs
//	printf("\n start halo exchange!!! \n");

  // Top of the Halo Exchange
	MPI_Isend(&(local->pixels[0][image->height*(image->height/nprocs)]),image->width*3, MPI_LONG_LONG, pabove, 0, MPI_COMM_WORLD, &req[0]);
	MPI_Irecv(&(local->pixels[0][0]), image->width*3, MPI_LONG_LONG, pbelow, 0, MPI_COMM_WORLD, &req[1]);

// Bottom of the Halo Exchange	
	MPI_Isend(&(local->pixels[0][image->height]), image->width*3, MPI_LONG_LONG, pbelow, 1, MPI_COMM_WORLD, &req[2]);
	MPI_Irecv(&(local->pixels[0][image->height*((image->height/nprocs)+1)]), image->width*3, MPI_LONG_LONG, pabove, 1, MPI_COMM_WORLD, &req[3]); 
